...
def main(gamma=1.0):
    for folder in os.listdir(src_dir):
        ...
        if video_num != 'video1':
            continue
        print(f"Processing images of {video_num}")
        print("="*20)
        current_folder = os.path.join(src_dir, folder)
        for file in os.listdir(current_folder):
            ...
            # 2) Apply gamma (still BGR)
            img_bgr_gamma = adjust_gamma(img_bgr, gamma=gamma)

            # 3) Convert BGR → RGB
            img_rgb = cv2.cvtColor(img_bgr_gamma, cv2.COLOR_BGR2RGB)
            ...
            if not (os.path.exists(f'../data/frames/processed/images/val')):
                os.makedirs(f'../data/frames/processed/images/val')
            cv2.imwrite(os.path.join(f'../data/frames/processed/images/val', file), img_out)
